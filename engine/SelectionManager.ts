...
export class SelectionManager {
  ...
  public initTicker() {
    this.engine.app?.ticker.add(() => {
      if (this.activeSelection) {
        this.dashOffset = (this.dashOffset + 0.15) % 8;
        this.drawMarquee();
      }
    });
  }
  ...
  public startMoving() {
    ...
    if (!this.activeSelection) {
      console.log('[StartMoving] ABORT: No activeSelection');
      return;
    }
    if (this.floatingPixels) {
      console.log('[StartMoving] ABORT: floatingPixels already exists');
      return;
    }

    const { x, y, width, height } = this.activeSelection;
    console.log('[StartMoving] Lifting pixels from', x, y, 'size', width, 'x', height);
  ...
  private createPreviewSprite() {
    ...
    const imageData = new ImageData(buffer, ow, oh);
