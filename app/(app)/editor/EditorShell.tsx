...
export default function EditorShell() {
  const {
    ...
    triggerUndo, triggerRedo, triggerDeleteSelection, triggerFlipH, triggerFlipV, triggerRotate, triggerImport, triggerExport, triggerExportJSON,
    ...
    <div className="flex flex-col h-screen bg-[#404040] text-gray-200 font-sans text-xs select-none">
      ...
      <div className="flex items-center h-8 bg-[#505050] border-b border-black px-2 gap-2">
        <div className="flex items-center gap-1 border-r border-[#303030] pr-2">
          ...
          <IconButton icon="📜" title="Export JSON (Game Data)" onClick={triggerExportJSON} />
        </div>
        <div className="flex items-center gap-1 border-r border-[#303030] pr-2">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-10 flex flex-col items-center py-1 bg-[#505050] border-l border-black overflow-y-auto gap-0.5">
          ...
          <div className="h-px w-6 bg-gray-600 my-1" />
          <ToolButton active={tool === 'pivot'} onClick={() => setTool('pivot')} icon="🎯" title="Pivot Tool (P)" />
          <ToolButton active={tool === 'transform'} onClick={() => setTool('transform')} icon="🏗️" title="Transform Tool (T)" />
        </div>
      </div>
      ...
      <div className="h-56 flex flex-col bg-[#505050] border-t border-black">
        ...
        <div className="h-7 bg-[#404040] border-b border-black flex items-center px-1 gap-1">
          ...
          {/* Easing Selector */}
          <select
            className="h-5 bg-[#303030] text-[10px] border border-black outline-none text-gray-300 cursor-pointer hover:bg-[#454545]"
            onChange={(e) => triggerSetEasing(e.target.value)}
            title="Frame Easing (Curve)"
          >
            <option value="Linear">Linear</option>
            <option value="EaseInQuad">Ease In (Vào chậm)</option>
            <option value="EaseOutQuad">Ease Out (Ra chậm)</option>
            <option value="EaseInOutQuad">Smooth (Mượt)</option>
            <option value="BounceOut">Bounce (Nảy)</option>
          </select>
          ...
          <div className="flex-1 bg-[#303030] relative overflow-auto flex flex-col">
            ...
            <div className="flex h-6 bg-[#404040] border-b border-black sticky top-0 z-10 min-w-max">
              {Array.from({ length: totalFrames }).map((_, i) => (
                <div
                  key={i}
                  className={`w-8 h-full border-r border-black flex items-center justify-center text-[10px] cursor-pointer hover:bg-[#505050] transition-colors ${i === currentFrame ? 'bg-[#ffcc00] text-black font-bold' : 'text-gray-400'
                    }`}
                ...
                <div key={layer.id} className="flex h-8 border-b border-black/30">
                  {Array.from({ length: totalFrames }).map((_, fIndex) => {
                    ...
                    return (
                      <div
                        key={fIndex}
                        className={`w-8 h-full border-r border-black/30 flex items-center justify-center cursor-pointer relative ${isActiveFrame && isActiveLayer
